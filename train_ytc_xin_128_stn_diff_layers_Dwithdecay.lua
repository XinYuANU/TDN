...
opt = lapp[[
  -s,--save          (default "logs128_ytc16_stn_D_decay")      subdirectory to save logs
...
local highHd5 = hdf5.open('datasets/YTC_HR_rotate_v2.hdf5', 'r')
...
local lowHd5 = hdf5.open('datasets/YTC_LR_rotate_v2.hdf5', 'r')
...
--[[
-- convert into y
trainData_HR_Y = torch.FloatTensor(ntrain,1,128,128)
trainData_LR_Y = torch.FloatTensor(ntrain,1,16,16)
valData_HR_Y = torch.FloatTensor(nval,1,128,128)
valData_LR_Y = torch.FloatTensor(nval,1,16,16)

for i = 1, ntrain do
	trainData_HR_Y[{i}] = image.rgb2y(trainData_HR[i])
	trainData_LR_Y[{i}] = image.rgb2y(trainData_LR[i])
end
for i = 1, nval do
	valData_HR_Y[{i}] = image.rgb2y(valData_HR[i])
	valData_LR_Y[{i}] = image.rgb2y(valData_LR[i])	
end

print(valData_LR:size(), valData_HR:size())

torch.manualSeed(1)
for i = 1, ntrain do
	theta = (torch.rand(ntrain)-0.5)*math.pi
	local tmp = image.rotate(trainData_HR[i],theta[i])
	trainData_LR[{i}] = image.scale(tmp,16,16,'bicubic')
end
for i = 1, nval do
	theta = (torch.rand(nval)-0.5)*math.pi
	local tmp = image.rotate(valData_HR[i],theta[i])
	valData_LR[{i}] = image.scale(tmp,16,16,'bicubic')
end
--]]
